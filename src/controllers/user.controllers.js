...
const updateCoverImage = asyncHandler(async (req, res) => {
    ...
    // Retrieve user to get the old cover image URL
    const user = await User.findById(req.user?._id).select("coverImage");
    const oldCoverImageUrl = user?.coverImage;

    // Update user with the new cover image URL
    const updatedUser = await User.findByIdAndUpdate(
        req.user?._id,
    ...
    if (oldCoverImageUrl) {
        const isDeleted = await deleteFromCloudinary(oldCoverImageUrl);
        if (!isDeleted) {
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields:{
                ...
                // isSubscribed:{$in:[req.user?._id,"$subscribedTo"]}
                isSubscribed:{
                    $cond:{
